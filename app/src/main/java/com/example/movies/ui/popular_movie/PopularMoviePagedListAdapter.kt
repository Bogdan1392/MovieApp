...
class PopularMoviePagedListAdapter(val context: Context) : PagedListAdapter<Movie, RecyclerView.ViewHolder>(MovieDiffCallback()) {
    ...
    fun setNetworkState(newNetworkState: NetworkState) {
        ...
        if (hadExtraRow != hasExtraRow) {
            if (hadExtraRow) {
                notifyItemRemoved(super.getItemCount()) // hadExtraRow is true and hasExtraRow is false => remove the progress bar at the end
            } else {
                notifyItemInserted(super.getItemCount()) // hasExtraRow is true and hadExtraRow is false => add the progress bar at the end
            }
